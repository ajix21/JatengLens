...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
    <h3 class="font-semibold text-gray-700 flex items-center gap-2">
        <i class="fas fa-tag text-indigo-500"></i> Informasi Kategori
    </h3>
    <div>
        <label class="block text-sm font-medium text-gray-600 mb-1">Nama Kategori <span class="text-red-500">*</span></label>
        <input type="text" name="name" value="{{ old('name') }}" required placeholder="cth: Mahasiswa, Buruh, LSM..."
    ...
    <div x-data="{ color: '{{ old('color', '#6366f1') }}' }">
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="w-10 h-10 rounded-lg border border-gray-200 flex-shrink-0" :style="`background:${color}`"></div>
        </div>
    ...
    <div>
        ...
        <textarea name="description" rows="3" placeholder="Deskripsi singkat kategori ini..."
                  class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2.5 focus:ring-2 focus:ring-indigo-300 focus:outline-none resize-none">{{ old('description') }}</textarea>
    </div>
